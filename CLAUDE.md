...
The UI follows a **dark medical workstation** aesthetic (PACS-style, inspired by professional AI radiology platforms like Aidoc and Viz.ai), with a light mode option for bright environments.

### Theme Architecture

- **`src/theme.js`** — exports `DARK`, `LIGHT` (identical-shape token objects), and `ThemeCtx = createContext(DARK)`
...
### Color Tokens (`src/theme.js` — `T` object)
...
`src/components/ClinicalDecision.jsx` — Doctor-in-the-Loop recommendation card:
- Reads T from `useContext(ThemeCtx)` — fully theme-reactive
- **4 action buttons**: ✓ Accept · ✎ Modify · ✗ Override (requires typed reason) · ⊘ Bypass
- **Bypass** records `decision.type = 'bypass'` with pre-filled audit note; no user input required
- Recommendation card has a left colored border keyed to `synthesis.strength` (Strong=orange, Moderate=blue, Weak=gray)
- Rationale bullets use directional icons: ▲ orange for high-risk items, ▼ green for conservative, • cyan otherwise
- `onDecision` callback types: `'accept' | 'modify' | 'override' | 'bypass' | null` (null = revise)
